...
const checks = [
  ...
  ["更新日志固定显示 3.0.79/3.0.78/3.0.77 且无失效历史入口", sources["src/App.tsx"].includes("changelog.slice(0, 3)") && sources["src/App.tsx"].includes("官网固定展示最近三次正式更新") && sources["public/download/index.html"].includes('<meta name="zhichao-recent-releases" content="3.0.79,3.0.78,3.0.77" />') && ["知潮 3.0.79：", "知潮 3.0.78：", "知潮 3.0.77："].every((title, index, titles) => index === 0 || sources["src/App.tsx"].indexOf(titles[index - 1], sources["src/App.tsx"].indexOf("const changelog =")) < sources["src/App.tsx"].indexOf(title, sources["src/App.tsx"].indexOf("const changelog ="))) && sources["src/App.tsx"].includes("useHashNavigation()") && sources["src/App.tsx"].includes("scrollIntoView") && !sources["src/App.tsx"].includes("showAllChangelog") && !sources["src/App.tsx"].includes("查看更多更新")],
  ...
  ["3.0.79 首页发布身份与修复说明一致", ["const APP_VERSION = '3.0.79'", "知潮 3.0.79 修复更新", "页面导航与记账删除紧急修复", "时光海洋", "学习工具"].every((phrase) => sources["src/App.tsx"].includes(phrase))],
  ...
  ["下载确认页保留 3.0.79 正式包身份、非强制更新与备案", ["3.0.79", "versionCode 116", "233755800", "9EF8A329B7F065296FEAB93612E11C2C5A00D1AA43BBDBCEE3F5752B6719D576", "222.9 MB", "官方已核验", "签名与公网文件一致", "非强制更新", "最新更新：", "2026 年 8 月 13 日 01:50（北京时间）", "多个入口不再跳回搭子首页", "记账删除冲突可以继续处理", "访问知潮官网", "陕ICP备2026019822号", "陕公网安备61092802000137号"].every((phrase) => sources["public/download/index.html"].includes(phrase))],
