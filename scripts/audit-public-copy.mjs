...
const checks = [
  ...
  ["更新日志固定显示 3.0.80/3.0.79/3.0.78 且无失效历史入口", sources["src/App.tsx"].includes("changelog.slice(0, 3)") && sources["src/App.tsx"].includes("官网固定展示最近三次正式更新") && sources["public/download/index.html"].includes('<meta name="zhichao-recent-releases" content="3.0.80,3.0.79,3.0.78" />') && ["知潮 3.0.80：", "知潮 3.0.79：", "知潮 3.0.78："].every((title, index, titles) => index === 0 || sources["src/App.tsx"].indexOf(titles[index - 1], sources["src/App.tsx"].indexOf("const changelog =")) < sources["src/App.tsx"].indexOf(title, sources["src/App.tsx"].indexOf("const changelog ="))) && sources["src/App.tsx"].includes("useHashNavigation()") && sources["src/App.tsx"].includes("scrollIntoView") && !sources["src/App.tsx"].includes("showAllChangelog") && !sources["src/App.tsx"].includes("查看更多更新")],
  ...
  ["3.0.80 首页发布身份与记账修复说明一致", ["const APP_VERSION = '3.0.80'", "知潮 3.0.80 记账修复更新", "旧账打开就能改", "备注输入也不再被挡住", "不再把朵朵短评挤成半句话"].every((phrase) => sources["src/App.tsx"].includes(phrase))],
  ...
  ["下载确认页保留 3.0.80 正式包身份、非强制更新与备案", ["3.0.80", "versionCode 117", "233778480", "9EF63D4537DC378FD09F764B6775F88BD6B0817CD0F3D8EAD0436CC5BC760619", "222.9 MB", "官方已核验", "签名与公网文件一致", "非强制更新", "最新更新：", "2026 年 8 月 13 日 06:10（北京时间）", "编辑旧账保留原分类和账户", "备注可见、可删", "账单短评完整显示", "访问知潮官网", "陕ICP备2026019822号", "陕公网安备61092802000137号"].every((phrase) => sources["public/download/index.html"].includes(phrase))],
