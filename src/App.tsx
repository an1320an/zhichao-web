...
const features = [
  ...
  { icon: aiChatIcon, title: '朵朵规则聊天', desc: '保留文字聊天，由审核文案库、关键词规则与学习数据组合回应；当前不调用云端大模型' },
...
const faq = [
  ...
  {
    q: '我的数据存在哪里？会交给第三方吗？',
    a: '账号、学习记录与聊天内容保存在我们的服务器上，服务器目前位于中国香港，属于个人信息出境。当前规则聊天不调用云端大模型，也不会把聊天内容发送给第三方模型；如未来依法启用模型功能，我们会在启用前另行显著说明并取得所需授权。我们计划在 huaix.cn 完成备案后迁回中国内地。详见隐私政策。',
  },
  {
    q: '我能导出或删除我的数据吗？',
  ...
  },
  {
    ...
    a: '不能。规则回复、题目与解析都只用于学习辅助，不能替代正式教材、考试大纲和专业医学判断，更不能作为医疗、诊断、用药的依据。详见免责声明。',
  },
  {
    q: '朵朵是真人吗？我可以把它当心理咨询用吗？',
    a: '朵朵是软件中的自动回应程序，不是真人，也不具备任何医疗或心理咨询资质，不能替代专业帮助。如果你正处在情绪困境中，请联系全国统一心理援助热线 12356；如遇紧急情况请拨打 110 或 120。',
  },
]
...
const changelog = [
  {
    date: '2026-07-19',
    title: '朵朵切换为审核文案规则聊天',
    desc: '聊天界面与历史记录继续保留，回复改由服务端审核文案库、关键词规则和学习数据组合；当前不调用云端大模型。以后如具备合规条件，可由服务端受控切换回复引擎，无需重做聊天功能。',
  },
  {
    date: '2026-07-16',
  ...
  },
  {
    ...
    desc: '出于未成年人保护与审慎运营考虑，当前内测仅面向年满 18 周岁用户，注册需提供出生年月（已注册的老用户在新版 App 中也需补填，不补填将无法继续使用）；朵朵常驻自动程序与回复来源标识；新增连续使用满 2 小时提醒与过度依赖提示；对涉及自伤、自杀的表述提供即时求助热线（12356）并由人工跟进；新增「导出我的数据」与投诉举报入口。',
  },
  {
    date: '2026-07-08',
    title: 'Android 内测开启',
    ...
    <div className="page">
      ...
      <main>
        <section className="hero">
          <div className="hero-inner">
            ...
            <p className="ai-notice">
              📚 朵朵当前由审核文案库和规则自动回应，不调用云端大模型，也不是真人。内容仅供学习辅助，不能替代教材与专业医学判断。本服务仅面向年满 18 周岁的用户。
            </p>
        ...
        <section id="architecture" className="architecture">
          ...
          <div className="arch-backend">后端（Fastify + SQLite）── 审核文案库与规则引擎</div>
          <p className="arch-note">所有数据变更、聊天规则匹配与业务逻辑统一经过后端；云端模型通道当前关闭。</p>
        </section>
      ...
      <footer className="footer">
        ...
        <p className="ai-notice-footer">
          📚 朵朵当前使用审核文案库与规则自动回应，不调用云端大模型，也不是真人。内容仅供学习辅助，不构成医疗建议。
        </p>
        <p className="ai-notice-footer">
