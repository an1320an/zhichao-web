...
const releaseHighlights = [
  {
    label: '计划',
    title: '从目标到每日行动，专属学习计划成为学习主线',
    desc: '用不超过五步说明考试方向、日期、可用时长和薄弱科目，朵朵会结合已有学习证据给出阶段路线与今天可做的安排；证据不足时先建立基线，不虚构掌握情况。',
  },
  {
    label: '路径',
    title: '路线图会随真实进度、薄弱科目和目标日期自动调整',
    desc: '基础、强化、冲刺阶段会标出当前站点、距目标天数和各科进度；修改日期或产生新证据后重新安排节奏，但不承诺考试结果。',
  },
  {
    label: '维护',
    title: '服务器维护时，登录前后都能看见明确状态',
    desc: '登录、注册和已登录页面会读取同一条公开维护信息，显示原因、预计恢复时间并自动刷新；普通断网不会被误报成维护。',
  },
  {
    label: '体验',
    title: '更新说明更短，“给你的一封信”恢复正常打开',
    desc: 'App 弹窗只保留一段摘要，完整内容继续放在官网下载页；同时修复欢迎信点击后返回搭子主页的问题，并补充私密记录的合理使用边界。',
  },
]
        ...
        <section className="release-story" aria-labelledby="release-story-title">
          <div className="release-story-heading reveal">
            <div className="release-story-meta">
              <span>知潮 3.0.75 正式更新</span>
              <time dateTime={APP_RELEASED_AT}>最新更新 · {APP_RELEASED_AT_LABEL}</time>
            </div>
            <h2 id="release-story-title">目标、路线图和每日行动，现在连成一条看得见的学习主线</h2>
            <p>专属学习计划会跟随真实证据调整；服务器维护时，登录前后也会显示清楚的状态与恢复信息。</p>
          </div>
